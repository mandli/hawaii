...
def setrun(claw_pkg='geoclaw'):
    ...
    clawdata.upper[0] = -120.0      # east longitude
    ...
    # ~ [27,500, 13750, 6875, 3437.5, 572.9166667, 35.80729167]
    ...
    rundata.gaugedata.gauges.append([1, -155.929991, 19.864890,
                                     rundata.clawdata.t0,
...
def setgeo(rundata):
    ...
    refine_data.speed_tolerance = [0.5, 1.0, 1.2]
    ...
    topo_path = os.path.join(os.environ["DATA_PATH"], "topography", "GEBCO", 
                             "GEBCO_2023.nc")
    ...
    clawutil.data.get_remote_file(
                   "https://ftp.nhc.noaa.gov/atcf/archive/2014/bep092014.dat.gz")
    atcf_path = os.path.join(scratch_dir, "bep092014.dat")
    ...
    iselle = Storm(path=atcf_path, file_format="ATCF")

    # Calculate landfall time - Need to specify as the file above does not
    # include this info - Landfall on big island (08/08/2014 - 12:30 UTC)
    iselle.time_offset = datetime.datetime(2014, 8, 8, 12)

    # Modify storm
    # Move storm N by 0.1 degrees
    iselle.eye_location[:, 1] += 0.5
    # Strengthen storm
    iselle.max_wind_speed *= 1.2
    # Modify sea-level
    geo_data.sea_level += 0.5

    iselle.write(data.storm_file, file_format='geoclaw')
